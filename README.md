...
- **实时界面**：SSE推送总量和变更会话绝对值，200ms合并通知；独立5秒心跳区分网页连接、采集器和数据同步。完整整数立即更新，M仅辅助，保留展开状态和选区。
...
| 在线采集器 | 根据服务器接收心跳的年龄判断；默认12秒待确认、30秒失联，不根据Token是否增长判断 |
| 等价价格 | 按本地价格规则计算的参考值，不代表实际扣款 |

有明确运行证据但无可见delta显示“执行中，等待usage”；证据不足显示“状态未知”，明确完成才显示空闲。可见输出只作本地估算，不是严格下界、不猜隐藏推理、不入精确账本。详见[实时同步说明与实测](docs/REALTIME.md)。系统不会伪造增长，也不会调用模型估算。
